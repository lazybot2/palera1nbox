...
#command chk
if '755' != oct(os.stat(f'{bash_path}checkra1n').st_mode)[-3:]:
    print('755')
    os.chmod(f'{bash_path}checkra1n',0o755)
else:
    print('command is OK')
# Options de menu et état
menu_options = {
    'main': ['Options', 'Run Checkra1n', 'Stop Checkra1n', 'Exit'],
    'options': ['Safe Mode', 'Verbose', 'Quick Mode', 'Revert', 'Back']
}
current_menu = 'main'
cursor_position = 0
    ...
    try:
        ...
        for line in process.stdout:
            ...
            if "All Done" in line:
                time.sleep(2)
                exit_program()
            elif out_en and 'USB' in line:
                time.sleep(2)
                exit_program()
            elif 'Booting' in line:
                out_en=True
            elif 'Exploiting' in line:
                out_en=False
    except Exception as e:
...
def display_message_scroll(lines):
    ...
    for i, line in enumerate(lines[start_line:start_line +6]):
        draw.text((0, i * 10), line, font=font_small, fill=255)
    oled.drawImage(image)
